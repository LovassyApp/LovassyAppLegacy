...
const Products = () => {
    ...
    const deleteRow = async (row) => {
        const res = await deleteModal('Biztos, hogy törölni szeretnéd ezt a terméket?', 'Törlés - ' + row.name, theme);

        if (res.isConfirmed) {
            toast.promise(
                client.products.delete(row.id).then(() => bootstrap()),
                {
                    loading: 'Törlés...',
    ...
    const columns = [
        ...
        },
        {
            name: 'Kód kell?',
            cell: (row) => {
                return Boolean(row.codeActivated) ? <MdCheck fontSize={20} /> : <MdClose fontSize={20} />;
            },
            maxWidth: '10px',
        },
        {
            ...
            cell: (row) => {
                return Boolean(row.visible) ? <MdCheck fontSize={20} /> : <MdClose fontSize={20} />;
            },
        },
        {
            name: 'Ár (LoLó)',
        ...
        },
        {
            ...
            cell: (el) => {
                return (
                    <>
                        <Button className="mx-1" auto rounded color="primary" onClick={() => editEl(el.id)}>
                            Szerkesztés
                        </Button>
