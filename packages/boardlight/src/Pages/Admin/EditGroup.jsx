...
const EditGroup = () => {
  ...
  const getChecked = (permissionString) => {
    return checked.includes(permissionString);
  };

  const togglePermission = (permissionString, newStatus = null) => {
  ...
  React.useEffect(() => {
    client.scopes
      .all()
      .then((res) => {
        setPermissions(Object.values(res));
        if (id !== "new") {
          client.groups
            .get(id)
            .then((res) => {
  ...
  return (
    <AuthLayout>
      <HeaderCard
        title={loading ? <Loading color="white" /> : "Csoport: " + name}
      />
      {loading ? (
        <center>
          <TableLoader />
                          ...
                          {el.permissions.map((el, key) => (
                            <Col md="6" className="mt-2" key={key}>
                              ...
                              >
                                ...
                                <p>{el.description}</p>
                                <Switch
                                  checked={getChecked(el.permissionString)}
                                />
                              </NextUICard>
                            </Col>
                          ))}
